...
def MinDiamSolNew(G: nx.Graph, AllNodeList: List[Node], FocalNodeID: int, FocalTask: Task, MAXIMUM_HOP: int):
    '''
    In this algorithm, rather than directly filtering nodes out by the MAXIMUM_HOP, we start with hop = 1 to MAXIMUM_HOP.
    '''
    FeasibleTeam = None
...
def MinAggrSolNew(G: nx.Graph, AllNodeList: List[Node], FocalNodeID: int, FocalTask: Task, MAXIMUM_HOP: int):
    '''
    In this algorithm, rather than directly filtering nodes out by the MAXIMUM_HOP, we start with hop = 1 to MAXIMUM_HOP.
    '''
    FeasibleTeam = None
